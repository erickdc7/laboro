...
def get_top_technologies(limit: int = 10, db: Session = Depends(get_db)):
    results = (
        db.query(JobTechnology.technology, func.count(JobTechnology.id).label("count"))
    ...
    return [{"technology": row.technology, "count": row.count} for row in results]


@router.get("/modality")
def get_modality_distribution(db: Session = Depends(get_db)):
    results = (
        db.query(Job.modality, func.count(Job.id).label("count"))
        .filter(Job.is_active == True)
...
def get_jobs_per_day(days: int = 30, db: Session = Depends(get_db)):
    ...
    results = (
        db.query(
            func.date(Job.scraped_at).label("date"), func.count(Job.id).label("count")
        )
...
def get_top_companies(limit: int = 10, db: Session = Depends(get_db)):
    ...
    return [{"company": row.company, "count": row.count} for row in results]
...
def get_summary(db: Session = Depends(get_db)):
    ...
    remote_count = (
        db.query(Job).filter(Job.is_active == True, Job.modality == "remote").count()
    )
    remote_percentage = (
        round((remote_count / total_jobs) * 100) if total_jobs > 0 else 0
    )
    return {
        ...
        "top_technology_count": top_tech.count if top_tech else 0,
